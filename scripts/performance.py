...
def _get_perf_data(bin_path: Path, function_name: str, exp_count: int, compilation_profile: str, device_name: str, result_directory: Path) -> Path:
    ...
    args = f'perf record -F 99 -e cpu-clock -a -g --call-graph fp --fsynchronous-unwind-tables -o "{perf_data_path}" -- "{bin_path}" {exp_count} {function_name} {sizes} experiments.log perf'
    _run_performance_binary("Perf record", args)
    return perf_data_path


def _get_perf_file_beginning(perf_data_path: Path) -> str:
...
def _create_flame_graph(perf_script_path: Path, perf_file_beginning: str, flamegraph_repo: Path, is_icicle: bool):
...
def _measure_performance_for_function(function_name, core_indeces, min_frequenciy, max_frequency, bin_path, exp_count, compilation_profile, device_name, flamegraph_repo, result_directory):
    interrupted = False
    try:
    ...
    perf_beginning = _get_perf_file_beginning(perf_data_path)
    perf_script_path = _get_perf_script(perf_data_path, perf_beginning, flamegraph_repo)
    _create_flame_graph(perf_script_path, perf_beginning, flamegraph_repo, is_icicle=False)
    _create_flame_graph(perf_script_path, perf_beginning, flamegraph_repo, is_icicle=True)


def measure_performance(optimization_classes: set[str], compilation_profiles: list[str], exp_count: int,
                        device_name: str, output_dir: str, suffix: str, flamegraph_repo: Path, no_recompile: bool, eigen_path: Path) -> list[str]:
