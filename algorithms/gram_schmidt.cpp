...
void vector_matrix_product(const num_type* vec, const num_type* transposed_matrix, num_type* result_vec, size_t row_count, size_t column_count){  
    for(size_t vec_index = 0; vec_index < row_count; ++vec_index){
        result_vec[vec_index] = inner_simple_dot_product(&transposed_matrix[vec_index*column_count], vec, column_count);
    }
}
...
vector_num gram_schmidt_full_matrix(vector_num& transposed_matrix, size_t row_count, size_t column_count){
    ...
    for(size_t vec_index=1;vec_index<row_count;++vec_index){
        square_ort_reciprocals[vec_index-1] = 1.0 / inner_simple_dot_product(&orthogonal_matrix[(vec_index-1)*column_count], &orthogonal_matrix[(vec_index-1)*column_count], column_count);
        ...
        for(size_t proj_index=0;proj_index<vec_index;++proj_index){
            num_type* projection = new num_type[column_count];
            inner_multiply_vector_by_number(&orthogonal_matrix[proj_index*column_count], projection, multipliers[proj_index], column_count);
...
vector_num block_gram_schmidt_matrix_inline_common(vector_num& transposed_matrix, size_t row_count, size_t column_count, dot_product_function dot_foo, sub_function sub_foo, number_mult_function mult_foo, size_t block_size, bool enable_parallel = false) {
    check_matrix(transposed_matrix, row_count, column_count);
    vector_num orthogonal_matrix = transposed_matrix;
    vector_num dot_product_results(row_count, 0.0);
...
vector_num gram_schmidt_block(vector_num& transposed_matrix, size_t row_count, size_t column_count) {
    return block_gram_schmidt_matrix_inline_common(transposed_matrix, row_count, column_count, inner_dot_product_unrolling, sub_vector_from_vector_inplace_unrolling, inner_multiply_vector_by_number_unrolling, kBlockSize, true);
}